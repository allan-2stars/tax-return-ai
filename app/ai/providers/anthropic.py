...
class AnthropicProvider(AIProvider):
    ...
    async def classify(
        ...
        if not self.api_key:
            raise AIProviderConfigurationError("Anthropic API key is missing.")
        try:
            import anthropic
            ...
            msg = str(e).lower()
            if any(k in msg for k in ["authentication", "api key", "unauthorized", "could not resolve authentication"]):
                raise AIProviderConfigurationError("Anthropic API credentials are invalid or missing.") from e
            raise AIProviderError(f"Anthropic provider failure: {e}") from e
